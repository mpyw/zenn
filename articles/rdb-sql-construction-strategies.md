...
厳密には，オプティマイザには **OR 展開**という抜け道が 1 つあります（`(:name IS NULL) OR (name = :name)` を，NULL 用の全表走査ブランチと非 NULL 用のインデックスブランチの `UNION ALL` に書き換える手）。ただしこれは条件が 1〜2 個のうちだけで，3 個を超えると組み合わせ爆発で破綻します（後の Oracle 節で実測とともに扱います）。**多条件検索という catch-all 本来の用途では，全表走査に落ちる**と考えてよいです。
...
そして **この形のままでは `name` のインデックスは使えません。** インデックスが使えるのは述語が `name = 何か` の形をしているときだけですが，この式は「`:b1` が NULL なら全行が該当する」という可能性を含んだ OR だからです。計画としては，全行を読んで 1 行ずつ判定するしかありません。
...
ほぼすべての方式・ライブラリで，**実行せずにプレースホルダ入りの SQL 文字列を取り出せます**（ORM でもクエリビルダーでもテンプレートでも同じ）。これを使い，条件の組み合わせを列挙して SQL を吐き出し，スナップショットファイルとしてコミットします。
...
- **テストが値に依存して壊れない。** 値ではなく構造だけをスナップショットするので，バインドされる値が変わっても差分は出ない
