...
3 段階目のファイルスコープはありません。フラットなパッケージでは，どんなに小さなヘルパーもパッケージ全体から見える名前で，どんなフィールドもパッケージ全体から書き換えられます。
...
そして Agentic AI 全盛期の今，この暗黙の了解から綻びが生まれやすくなりました。エージェントはスコープに見えている unexported なヘルパーを見つけたら，素直に呼びます。unexported なフィールドが見えていたら，素直に書き込みます。そのどれもがコンパイルを通り，レビューが雑であれば通過してしまいます。コメントである程度軽減は可能とはいえども，完全ではありません。 AI は書くスピードが非常に速いので，負債が積み上がるスピードも段違いです。
...
| ルール | 何を問うか | 既定 |
|:---|:---|:---|
| **`boundary`** | このファイルは，あの宣言に触ってよいか | 既定で ON |
| **`qualify`** | その名前だけを読んで，どの関心範囲のものか分かるか | 既定で OFF |
...
:::message
`boundary` は v0.5.0 で `rules.allowBoundary: true` を書くと切れるようになりました。**「スコープという概念までは要らないが，命名規則は欲しい」** というリポジトリのためのものです。スコープはディレクティブを書いて維持する手間がかかるので，そこまでは払わずに「どの単位のものかが名前で読める」だけが欲しい，という判断はあり得ますよね。

ただし **段階導入の手段ではありません。** それは後述の baseline の仕事です。baseline は既存を黙らせつつ **新規は報告する** のに対し，スイッチは何も報告しないので，ON に戻すコストが永遠に測られないままになります。
:::
...
既定で OFF ですが，**私は ON にすることをおすすめします。** パッケージレベルの宣言の名前に，そのファイルの namespace をどこかに含めることを要求します。 `boundary` が 「このファイルはあの宣言に触ってよいか」 を問うのに対し，`qualify` は **「その名前だけを読んで，どの関心範囲のものか分かるか」** を問います。

:::message
プレフィックスの強制ではありません。プレフィックスを強制すると不自然な名前になってしまうことも多いため，柔軟性を重視しています。
:::
...
- **ディレクティブは，人間が下した判断の永続的な記録になる。** 次のエージェントはそれを導出し直さずに継承する
